...
void moveTopofBlock(Konfiguration &curr, koord topBlockPos, int topBlocks, koord targetBlPos, koord targetPos, int targetBlock, int &counter);
void moveTopofPlace(Konfiguration &curr, koord topBlockPos, int topBlocks, koord targetBlPos, koord targetPos, int targetBlock, int &counter);
...
//---------Eingabe der Konfigurationen-------------------------
    ...
    while(blocksSorted != goal.anzahlBl){
        ...
        if(tBlPos.x!=tPos.x && tBlock!=0){
            ...
            moveTopofBlock(curr,topBlockPos,topBlocks,tBlPos,tPos,tBlock,stepCount);


            topBlockPos = curr.searchBlock(curr.nameOfBlock(tPos.x));    //top block position (gleicher platz wie tPos)
            topBlocks = topBlockPos.y-tPos.y;   //menge der bloecke auf unseren target Place

            moveTopofPlace(curr,topBlockPos,topBlocks,tBlPos,tPos,tBlock,stepCount);
